        ...
        {profiles.map((p) => (
          <div key={p.id} className="list-item" style={{ boxShadow: 'none', background: '#fffdf5' }}>
            ...
            <span className="body">
              ...
              <div className="sub">{p.role === 'parent' ? 'Ouder' : 'Kind'}{p.role === 'kid' ? ` · weergave ${p.density === 'simple' ? 'simpel' : 'normaal'}` : ''}{p.hasPin ? ' · 🔒 pincode' : ''}</div>
            </span>
...
function ProfileForm({ initial, onDone }: { initial?: Profile; onDone: () => void }) {
  ...
  const [role, setRole] = useState<'kid' | 'parent'>(initial?.role ?? 'kid');
  ...
  async function submit(e: React.FormEvent) {
    ...
    const body: Partial<Profile> & { pin?: string | null } = { name: name.trim(), avatar, color, role, density };
    if (pin) body.pin = pin; else if (clearPin) body.pin = null;
    try {
  ...
  return (
    <form className="form" onSubmit={submit}>
      ...
      <label>{initial?.hasPin ? 'Nieuwe pincode (leeg = laten staan)' : 'Pincode (optioneel voor kind, verplicht voor ouder)'}
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        {initial && <button type="button" className="btn btn-bad" onClick={async () => { if (confirm(`${initial.name} verwijderen? Alle kaarten gaan mee.`)) { try { await api.deleteProfile(initial.id); onDone(); } catch (err) { setError(err instanceof Error ? err.message : 'Lukt niet'); } } }}>🗑️</button>}
